...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!clientId || !clientSecret) {
      return res.status(500).json({ error: 'Zoom OAuth not properly configured' })
    }
    ...
    if (req.method === 'GET') {
      const successUrl = process.env.NEXTAUTH_URL + '/dashboard?zoom_connected=true'
    ...
    if (req.method === 'GET') {
      const errorUrl = process.env.NEXTAUTH_URL + '/dashboard?zoom_error=true'
